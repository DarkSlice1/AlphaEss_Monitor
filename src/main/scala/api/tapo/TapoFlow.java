...
import api.tapo.domain.*;
import api.tapo.helpers.KspB64;
import api.tapo.helpers.KspDebug;
import api.tapo.helpers.KspHttp;
import api.tapo.helpers.KspJson;
...
public class TapoFlow {
    ...
    public Integer getPlugEnergyUsage(C658a c658a, String token, String cookie){
        try{
            DeviceInfoParams deviceInfoParams = new DeviceInfoParams();
            TPIoTRequest<DeviceInfoParams> tpIoTRequest = new TPIoTRequest<>();
            tpIoTRequest.setMethod("get_energy_usage");
            tpIoTRequest.setParams(deviceInfoParams);
            tpIoTRequest.setRequestTimeMils(System.currentTimeMillis());
            tpIoTRequest.setTerminalUUID("88-54-DE-AD-52-E1");
            ...
            KspDebug.out("Decrypted: " + dec);
             ...
             return new GetEnergyResponse(
                    cookie,
                    KspJson.convertToObj(dec)
            ).getResponse().get("result").getAsJsonObject().get("current_power").getAsInt();
        }
        catch (Exception ex)
        {
            KspDebug.out("Something went wrong! " + ex.getMessage());
