...
import Api.TapoObjectMapper.{C658a, HandShakePayload, HandShakePayloadParams, HandShakeReply, KspB64, KspKeyPair, LoginPayload, LoginPayloadParams, SecurePassthroughPayload, SecurePassthroughPayloadParams}
...
class Tapo {
  ...
  val keyPair = new KspKeyPair(privateKey, publicKey);
  var Cookie = ""
  ...
  def decode_handshake_key(key:String): C658a = {
    try {
      val decode: Array[Byte] = new KspB64().decode(secretKey.getBytes("UTF-8"));
      val decode2: Array[Byte]= new KspB64().decode(keyPair.getPrivateKey())
